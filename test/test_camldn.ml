  ...
  Testo.create "maybe_do caches on miss, returns on hit" (fun () ->
    with_test_cdn (fun _t ->
      ...
      let f s =
        ...
        String.length s
      in
      let r1 = IntCache.maybe_do ~f_name:"len" ~f "hello" in
      let r2 = IntCache.maybe_do ~f_name:"len" ~f "hello" in
...
let test_different_params =
  Testo.create "different params get different cache entries" (fun () ->
    with_test_cdn (fun _t ->
      ...
      let r1 = IntCache.maybe_do ~f_name:"len" ~f "hi" in
      let r2 = IntCache.maybe_do ~f_name:"len" ~f "hello" in
...
let test_different_f_names =
  Testo.create "different f_names are separate cache keys" (fun () ->
    with_test_cdn (fun _t ->
      let call_count = ref 0 in
      let f _s = incr call_count; 42 in
      let r1 = IntCache.maybe_do ~f_name:"alpha" ~f "x" in
      let r2 = IntCache.maybe_do ~f_name:"beta" ~f "x" in
      Testo.(check int) ~msg:"alpha" 42 r1;
      Testo.(check int) ~msg:"beta" 42 r2;
      Testo.(check int) ~msg:"called twice (different f_names)" 2 !call_count))
...
let test_marshal_cache =
  Testo.create "MakeMarshal round-trips values" (fun () ->
    with_test_cdn (fun _t ->
      ...
      let r1 = MarshalCache.maybe_do ~f_name:"upper" ~f "meow" in
      let r2 = MarshalCache.maybe_do ~f_name:"upper" ~f "meow" in
